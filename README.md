# 客服中转中心（Service Router）

电商客服智能中转路由系统，基于 PyTorch JointBERT 模型实现意图识别与槽位填充，支持多轮对话澄清、服务渠道路由、人工审核确认等功能。

## 功能特点

- **NLP 能力**：规则匹配 + BERT 模型双轨意图识别，jieba 中文分词
- **意图分类**：覆盖交易售后（12类）、物流配送（7类）、票据优惠（6类）全场景
- **槽位提取**：自动识别订单号、手机号、商品名、收货地址、发票抬头等关键信息
- **置信度判定**：高置信直接跳转、中置信追问澄清、低置信转人工
- **多轮对话**：最多 5 轮对话澄清（长对话模式可达 50 轮），避免单句断章取义
- **人工审核**：支持开启人工复核模式，管理员确认后才执行分发
- **长对话模式**：可切换至 50 轮深度对话模式，适应复杂咨询场景
- **API 预留**：完整的 API 转发配置页面，便于后续对接真实接口
- **智能学习**：识别不出商品时自动反问用户，学习并保存新商品名称
- **上下文理解**：利用会话历史辅助意图识别，避免重复追问
- **商品清洗**：自动去除量词（个、件、套）和形容词（漂亮、蓝色）提取核心商品名
- **意图追踪**：记录用户意图变化轨迹，支持上下文联想
- **智能检测**：自动检测重复表达、确认意图、否定意图，精准把握用户真实需求
- **会话压缩**：自动压缩会话历史，节省内存同时保留关键信息

## 技术架构

```
┌─────────────────────────────────────────────────────────────┐
│                        前端层                               │
│   用户输入 Tab | 管理员审核 Tab | API 转发 Tab              │
│   (防重复提交按钮状态管理)                                  │
├─────────────────────────────────────────────────────────────┤
│                        应用层                               │
│                    server.py (Flask)                        │
│   - 路由处理 / 会话管理 / 审核流程 / 学习流程                │
├─────────────────────────────────────────────────────────────┤
│                        业务层                               │
│   NLPService | SessionService | ReviewService               │
│   (意图识别 / 会话管理 / 审核管理)                           │
├─────────────────────────────────────────────────────────────┤
│                        核心层                               │
│   ServiceRegistry | Models | Inference                      │
│   (服务注册 / 数据模型 / 预测引擎)                           │
├─────────────────────────────────────────────────────────────┤
│                        模型层                               │
│   JointBERT (BERT + 意图分类 + CRF 槽位标注)                │
└─────────────────────────────────────────────────────────────┘
```

## 项目结构

```
zryy/
├── main.py                         # 启动入口
├── README.md                       # 项目说明
├── LICENSE                         # 许可证
├── requirements.txt                # 依赖清单
├── stopwords.txt                   # 停用词
├── data/                           # 训练数据集（JDDC真实数据）
│   ├── train.json                  # 训练数据 (36,398条)
│   ├── val.json                    # 验证数据 (7,799条)
│   ├── test.json                   # 测试数据 (7,801条)
│   ├── train_token.json            # 训练token数据
│   ├── val_token.json              # 验证token数据
│   └── test_token.json             # 测试token数据
├── checkpoints/                    # 模型权重
│   └── best_model.pt               # 训练好的最佳模型
├── src/
│   ├── __init__.py                 # 模块导出
│   ├── models.py                   # 核心数据模型
│   ├── registry.py                 # 服务注册中心
│   ├── api/                        # REST API服务
│   │   ├── __init__.py
│   │   └── server.py               # 主服务器（Flask）
│   ├── config/                     # 配置文件
│   │   └── config.py               # 模型/意图/槽位配置
│   ├── data/                       # 学习数据
│   │   └── learned_products.json   # 学习到的商品列表
│   ├── model/                      # 机器学习模型
│   │   ├── __init__.py
│   │   ├── joint_bert.py           # JointBERT架构
│   │   ├── data_loader.py          # 数据加载与预处理
│   │   ├── inference.py            # 模型推理
│   │   └── train.py                # 模型训练
│   ├── services/                   # 业务服务
│   │   ├── nlp_service.py          # NLP意图识别与槽位提取
│   │   ├── session_service.py      # 会话创建与消息管理
│   │   └── review_service.py       # 人工审核流程管理
│   └── utils/                      # 工具模块
│       ├── errors.py               # 异常类定义
│       ├── hooks.py                # 钩子机制
│       └── tools.py                # 通用工具类
```

## 核心模块

### src/models.py

定义系统核心数据模型（采用现代 Python 架构风格，使用 frozen dataclass 确保数据不可变、可变 dataclass 支持状态流转）：

| 模型 | 说明 |
|------|------|
| `MessageRole` | 消息角色枚举（USER / ASSISTANT） |
| `ChannelGroup` | 渠道分组枚举（交易售后/物流配送/票据优惠/兜底） |
| `ServiceChannel` | 服务通道定义（frozen dataclass） |
| `Session` | 会话管理 |
| `Message` | 消息结构 |
| `Prediction` | 意图预测结果（含置信度、槽位、澄清问题） |
| `ReviewItem` | 审核项（可变 dataclass，支持状态变更） |
| `AppError` | 异常基类 |

### src/registry.py

服务注册中心（采用注册器模式，统一管理服务通道的注册、查询与路由）：

- `register()` 注册服务通道及其关键词映射
- `get_channel()` / `get_channel_by_intent()` 按名称/意图查询
- `find_channels_by_keyword()` 关键词匹配路由
- `get_channels_by_group()` 按分组查询

### src/services/nlp_service.py

NLP 服务模块：

- **意图识别**：规则匹配 + 模型预测双轨融合
- **槽位提取**：正则匹配订单号、手机号、地址等
- **商品识别**：三级识别（精确匹配、模式匹配、后缀匹配）+ 智能学习
- **商品清洗**：自动去除量词和形容词，提取核心商品名
- **上下文理解**：利用会话历史辅助意图识别
- **澄清问题**：每种意图预置 3 条多样化问题，随机选取
- **置信度判定**：高(≥0.85) / 中(≥0.4) / 低 三档

### src/api/server.py

REST API 服务器模块：

- **路由处理**：用户对话、审核管理、API 转发等接口
- **会话管理**：创建、查询、重置会话
- **审核流程**：待审核列表、确认分发
- **学习流程**：商品学习、槽位更新

### src/services/session_service.py

会话服务模块：

- 会话创建与持久化管理
- 消息添加与历史查询
- 用户消息计数（用于控制对话轮次）
- 多轮对话汇总（用于最终意图识别）
- 槽位信息自动更新（支持上下文继承）
- 元数据管理（用于追踪学习状态、最后意图等）

### src/services/review_service.py

审核服务模块：

- 待审核列表管理（添加、移除、查询）
- 审核操作（确认分发、修改意图分类）
- 聊天记录格式化展示
- 审核统计

## 架构优化亮点

### 1. 注册器模式（Registry Pattern）

`ServiceRegistry` 采用中心化注册机制，所有服务通道通过 `register()` 统一注册，支持：
- 按名称/意图快速查询
- 关键词匹配路由
- 按分组管理服务

### 2. 不可变数据模型（Immutable Data）

核心数据对象使用 `frozen dataclass` 定义，确保数据不可变：
- `ServiceChannel`、`Prediction` 等对象创建后状态不会被意外修改
- 仅状态流转对象（如 `ReviewItem`）使用可变 dataclass

### 3. 服务层分离（Service Layer）

业务逻辑按功能拆分为独立服务模块：
- `NLPService`：意图识别与槽位提取
- `SessionService`：会话管理
- `ReviewService`：审核流程
- 各服务职责单一，易于测试和维护

### 4. 查询引擎模式（Query Engine）

`NLPService` 实现了双轨意图识别机制：
- 规则匹配：快速关键词匹配
- 模型预测：基于 BERT 的深度学习分类
- 结果融合：取置信度更高的结果

### 5. 结构化异常处理

定义了完整的异常体系：
- `AppError` 作为基类
- 各业务异常继承自基类
- 便于统一错误处理和日志记录

### 6. 智能商品学习

实现了完整的商品学习机制：
- 识别不出商品时自动反问用户
- 用户回答后自动清洗（去除量词、形容词）
- 清洗后的商品名持久化保存
- 后续对话自动识别学习过的商品

### 7. 上下文感知

增强了多轮对话的上下文理解能力：
- 利用会话历史辅助意图识别
- 自动继承历史槽位信息
- 追踪最后意图，避免重复追问
- 检测重复意图表达，减少机械重复

## 使用方法

### 1. 安装依赖

```bash
pip install -r requirements.txt
```

### 2. 启动服务

```bash
python main.py
```

访问地址：http://127.0.0.1:5000

系统会自动使用已训练的模型（`checkpoints/best_model.pt`）进行意图识别。

### 3. 训练模型（可选）

如果需要重新训练模型：

```bash
python src/model/train.py
```

训练完成后会自动生成 `checkpoints/best_model.pt`。

### 4. 重置对话

点击页面右上角"重置对话"按钮，开始新的对话。

## 界面说明

### 用户输入 Tab

- 智能客服聊天界面，机器人主动问候"亲，您有什么需求？"
- 用户输入需求后，AI 根据置信度进行追问或直接确认
- 显示剩余对话轮次（默认最多 5 轮，长对话模式可达 50 轮）
- 第 5 轮后总结并跳转到对应渠道（或强制跳转）
- 右上角可切换"长对话模式"（50轮）和"人工审核"开关
- 点击"重置对话"按钮，一键开始新对话

### 管理员审核 Tab

- 待审核列表展示（会话摘要、置信度、对话轮次）
- 完整聊天记录查看（格式化显示）
- 槽位信息标签展示
- 操作按钮：**确认分发**（含意图选择下拉框）

### API 转发 Tab

- 所有服务渠道的 API 配置信息
- 按分组展示（交易售后类 / 物流配送类 / 票据优惠类 / 兜底通道）
- 显示渠道名称、请求方式、API 地址、描述

## 数据集

本项目使用 **JDDC（京东客服对话数据集）**，包含 51,998 条真实用户客服对话。

### 数据分布

| 意图类型 | 数量 | 占比 |
|----------|------|------|
| 其他咨询 | 33,167 | 63.8% |
| 物流查询 | 7,682 | 14.8% |
| 查订单 | 3,870 | 7.4% |
| 退货申请 | 2,689 | 5.2% |
| 改地址 | 1,230 | 2.4% |
| 开发票 | 904 | 1.7% |
| 退款申请 | 706 | 1.4% |
| 价格补差 | 568 | 1.1% |
| 换货申请 | 388 | 0.7% |
| 质量问题 | 334 | 0.6% |
| 领优惠券 | 239 | 0.5% |
| 补发申请 | 219 | 0.4% |
| 商品投诉 | 2 | 0.0% |

### 数据格式

每条数据包含：
- `text`：用户输入文本
- `intent`：意图标签
- `slots`：槽位标注（BIO 格式）

## 意图分类体系

### 交易售后类（12个）

退货申请、退款申请、换货申请、拒收、售后维权、商品投诉、商家投诉、质量问题、货不对板、少发漏发、查订单、补发申请

### 物流配送类（7个）

物流查询、查快递、发货时效、丢件破损、改地址、改手机号、改配送

### 票据优惠类（6个）

开发票、改发票抬头、补开发票、领优惠券、店铺券、价格补差

### 兜底通道（2个）

转人工、default（未知意图兜底）

## 置信度判定机制

| 置信度 | 处理方式 |
|--------|---------|
| ≥ 0.85 | 高置信，AI 直接确认并跳转渠道 |
| 0.4 ~ 0.85 | 中置信，AI 主动追问澄清 |
| < 0.4 | 低置信，自动转人工 |

## 全局审核开关

- **关闭审核（默认）**：AI 识别后直接跳转对应渠道
- **开启审核**：识别结果推入管理员审核列表，需人工确认分发

## 智能学习机制

### 学习流程

1. 用户输入"我想退那个东西"（系统识别不出商品）
2. 系统反问："抱歉，我没能识别出您提到的商品。请告诉我您说的是什么商品呢？"
3. 用户回答："一个漂亮的书柜"
4. 系统清洗：去除量词"一个"和形容词"漂亮的"，提取"书柜"
5. 系统保存：将"书柜"写入 `src/data/learned_products.json`
6. 后续识别：用户说"书柜想退货"就能直接识别

### 商品清洗规则

自动去除：
- **量词**：个、件、套、只、双、条、台、瓶、盒、袋、款、类、种、样、些、多、各、每
- **形容词**：好、坏、新、旧、大、小、高、矮、长、短、胖、瘦、宽、窄、厚、薄、漂亮、好看、难看、可爱、实用、便宜、贵、划算
- **颜色**：红、绿、蓝、白、黑、黄、紫、粉、灰、棕
- **代词**：这个、那个、的、是、了、想

### 支持的商品类别

| 类别 | 示例商品 |
|------|---------|
| 数码电子 | 手机、电脑、耳机、充电宝、显示器 |
| 家居用品 | 柜子、桌子、椅子、床、沙发、台灯 |
| 服饰鞋包 | 衣服、鞋子、包包、帽子、围巾 |
| 食品饮料 | 零食、水果、饮料、牛奶、面包、饼干 |
| 玩具玩偶 | 娃娃、玩具、玩偶、公仔、积木、拼图 |
| 个人护理 | 牙刷、牙膏、毛巾、洗发水、沐浴露 |
| 厨房用具 | 锅、碗、盘子、筷子、勺子 |

## 扩展指南

### 添加新服务渠道

在 `src/registry.py` 的 `_build_default_registry()` 函数中添加：

```python
registry.register(ServiceChannel(
    name='新渠道名称',
    intent='新意图',
    group=ChannelGroup.TRANSACTION_AFTER_SALES,
    api='/api/services/new_channel',
    method='POST',
    description='描述信息',
    keywords=('关键词1', '关键词2'),
    required_slots=('订单号',),
))
```

### 添加澄清问题

在 `src/services/nlp_service.py` 的 `CLARIFICATION_QUESTIONS` 字典中添加：

```python
'新意图': ['问题1', '问题2', '问题3'],
```

### 添加新商品

直接在 `src/services/nlp_service.py` 的 `COMMON_PRODUCTS` 列表中添加：

```python
'新商品名',
```

或者让系统通过对话学习自动添加。

## 技术栈

- Python 3.9+
- Flask 2.0+
- PyTorch
- Transformers (bert-base-chinese)
- jieba（中文分词）

## License

MIT License